 ...
 * When this method is called, it will reset the DB locations collection
 * with the Seed data from (data.json)
 */
LocationSchema.statics.seedLocationModel = (Location) => {
  Location.deleteMany({}, (err) => {
    if (err) return logger.error(`${err} :: ${CURRENT_DATE}`);
    return LOCATION_MODEL_SEED_DATA.forEach((location) => Location.create(location));
  });
